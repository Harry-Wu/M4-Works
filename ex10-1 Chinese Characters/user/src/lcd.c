...
void lcd_port_init(void)
{
	...
	GPIOD->MODER &=~(0X03<<0 | 0X03<<2 | 0X03<<8 | 0X03<<10 | 0X03<<16 | 0X03<<18 | 0X03<<20 | 0X03<<28 | 0X03U<<30  );
	GPIOD->MODER |=0X01<<0 | 0X01<<2 | 0X01<<8 | 0X01<<10 | 0X01<<16 | 0X01<<18 | 0X01<<20 | 0X01<<28 | 0X01<<30;
	GPIOD->OSPEEDR |=0X03<<0 | 0X03<<2 | 0X03<<8 | 0X03<<10 | 0X03<<16 | 0X03<<18 | 0X03<<20 | 0X03<<28 | 0X03U<<30  ;
	//PE 7~15
	GPIOE->MODER &=~(0X03<<14 |0X03<<16  |0X03<<18  |0X03<<20  |0X03<<22 |0X03<<24 |0X03<<26 |0X03<<28 |0X03U<<30);
	GPIOE->MODER |=(0X01<<14 |0X01<<16  |0X01<<18  |0X01<<20  |0X01<<22 |0X01<<24 |0X01<<26 |0X01<<28 |0X01<<30);
	GPIOE->OSPEEDR |=(0X03<<14 |0X03<<16  |0X03<<18  |0X03<<20  |0X03<<22 |0X03<<24 |0X03<<26 |0X03<<28 |0X03U<<30);
	//PF 12
	GPIOF->MODER &=~(0X03<<24);
	GPIOF->MODER |=0X01<<24;
...
void LCD_ShowString(u16 x, u16 y, u8 *p, u8 mode)
{
	while(*p != '\0')
	{
		...
		else if((*p>= 0x0A1) && (*p<=0x0FE))
		{
			LCD_Show_CH(x, y, p, mode);  //p为区码的存放地址
			p = p+2;  //跳到下一个区码
			x = x+16;
		}
		else
...
/********************************************************************
//mode: 0:覆盖模式, 1:叠加模式
//用指针从w25q64中取汉字, 16*16像素, 总共32字节数据, 一次取32个字节,放到buf里
//汉字字符第一个字节为区码, 第二个字节为位码
//纵向取模方式二
//如何计算汉字在flash中的位置:
汉字在flash中第几个位置: (区码-第一个区码)*94 + (位码-第一个字符的位码)
一个汉字占32个字节,所以上面的式子要*32, 得出
汉字在flash中的第几个地址: ((区码-0xA1)*94 + (位码-0xA1))*32
********************************************************************/
void LCD_Show_CH(u16 x, u16 y, u8 *p, u8 mode)
{
	u8 i,j,buf[32],_data;  //buf[32]里存一个汉字的数据
	u16 color = POINT_COLOR;  //临时保存画笔颜色
	u16 y0=y;  //y0用来临时存放y初始值
	SPI_Flash_Read(buf,((*p-0xA1)*94 + (*(p+1)-0xA1))*32, 32);  //一次从flash取32个字节, 放到buf里面
	...
	for(i=0; i<32; i++)
	{
		...
		for(j=0; j<8; j++)
		{
			...
			else
			{
				if(mode==0)  //如果是覆盖模式
				{
					POINT_COLOR=BACK_COLOR;  //画笔改为背景色
					LCD_DrawPoint(x,y);  //覆盖成背景色
					POINT_COLOR=color;  //恢复画笔颜色
				}
			}
			//一个点处理完后进行的动作			
			y++;
			if(y-y0==16)  //判断一列是否结束，结束的话，x跳到第二列，y跳回到第一行
			{
				y = y0;
				x++;
			}
