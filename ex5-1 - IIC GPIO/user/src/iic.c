...
void iic_start(void)
{
	...
	delay_600ns();//根据24c02的tSU.STA, >0.6uS
	IIC_SDA_OUT = 0;
	delay_600ns();//根据24c02的tHD.STO, >0.6uS
	IIC_SCL = 0;		
}
...
void iic_stop(void)
{
	...
	IIC_SDA_OUT = 0;
	IIC_SCL = 1;
	delay_600ns();  //根据24c02的tSU.STO, >100ns
	IIC_SDA_OUT = 1;
	delay_600ns();  
	delay_600ns(); //根据24c02的tBUF, >1.2uS
}
...
u8 iic_read_byte(void)
{
	...
	{
		IIC_SCL = 0;
		delay_600ns();delay_600ns();//24c02的tLOW>1.2uS
		IIC_SCL = 1;
		rec_data = rec_data<<1;
		if(IIC_SDA_IN)
		{
