...
void time12_pwm_init(void)
{
	...
	GPIOB->OSPEEDR &= ~(0X03<<30);  //输出速度2M
	GPIOB->PUPDR &= ~(0X03<<30);  //无上下拉
	GPIOB->AFR[1] &= 0x0FFFFFFF;  //清零15位
...
//2.	设置ARR（自动重装载值）、PSC（预分频值）
	TIM12->ARR =1000-1;  
	TIM12->PSC =84-1;  //1KHz频率
	TIM12->CCR2 = TIM12->ARR>>1; //50%占空比
//3.	使能更新，并且设置UG位能产生更新事件
	TIM12->CR1 |=0X01<<7;	//ARR有缓存
	TIM12->CR1 &=~(0X01<<3); //不使用单脉冲模式
	TIM12->CR1 &=~(0X03<<1); //使能更新，设置UG位会产生更新
	
//通道2设置
	TIM12->CCMR1 &= 0X03<<8;  //输出模式
	TIM12->CCMR1 |= 0X01<<11;  //CCR2有缓存
	TIM12->CCMR1 &= ~(0x07<<12);
	TIM12->CCMR1 |= 0x06<<12;  //PWM1模式
	TIM12->CCER &= ~(0X01<<5);  //高电平有效
	TIM12->CCER |= 0X01<<4; //开启通道2
