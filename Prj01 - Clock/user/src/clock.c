...
void display_tim(void)
{
	if(t!=time_date.sec)
	{
		...
		if(KEY2_MODE && setting_mode>7 && setting_mode<=10) BACK_COLOR = GREEN;//如果是时间修改模式8-10
		sprintf((char*)tbuf,"%02d:%02d:%02d",week_alam.hour, week_alam.min, week_alam.sec); 
		LCD_ShowString(165,260,tbuf,0);
		BACK_COLOR = WHITE;
		if(KEY2_MODE && setting_mode==11) BACK_COLOR = GREEN;//如果是时间修改模式11
